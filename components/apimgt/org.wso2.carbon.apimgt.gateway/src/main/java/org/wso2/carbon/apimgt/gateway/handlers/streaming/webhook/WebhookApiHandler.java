...
public class WebhookApiHandler extends AbstractHandler {
    ...
    private static final String TOPIC_QUERY_PARAM_NAME = "topicName";
    private static final String EMPTY_STRING = "";
    private static final String WEB_HOOK_SUBSCRIPTION_FAILURE_HANDLER = "_web_hook_subscription_failure_handler";
    private static final String WEB_HOOK_SUBSCRIPTION_RESOURCE_PATH = "/subscription";

    @Override
    public boolean handleRequest(MessageContext synCtx) {

        // applicable only for subscription flow
        String requestSubPath = getRequestSubPath(synCtx);
        if (requestSubPath.startsWith(WEB_HOOK_SUBSCRIPTION_RESOURCE_PATH)) {
            String topicName = getTopicName(RESTUtils.getFullRequestPath(synCtx));
    ...
    private String getTopicName(String url) {
        ...
        if (queryIndex != -1 && url.contains(TOPIC_QUERY_PARAM_NAME)) {
            ...
            for (String entry : entries) {
                ...
                if (index != -1) {
                    try {
                        ...
                        if (name.equalsIgnoreCase(TOPIC_QUERY_PARAM_NAME)) {
